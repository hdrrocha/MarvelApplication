...
class CharactersViewModel @Inject constructor(val api: ApiClient, private val schedulers: SchedulerProvider) : ViewModel() {

    val _data = MutableLiveData<List<SuperHero>>()
    val data: LiveData<List<SuperHero>> = _data

    fun getMoreMovies(page: Long) {
     api.characters(MarvelApiApi.API_KEY, MarvelApiApi.API_HASH, page).subscribeOn(schedulers.io())
             .observeOn(schedulers.mainThread())
             .subscribe({

                 Log.i("Helder ===> ", it.data.results[1].name)
